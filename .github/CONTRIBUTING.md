# Contributing to OpenStack Plugin
...
runtime in order to build packer with the OpenStack plugin.
...
With Go installed, you can already `go get` the OpenStack plugin and `make dev` in
...
3. One of OpenStack plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the OpenStack plugin
project. After following the steps in "Setting up Go to work on the OpenStack plugin":
...
Please only apply the minimal vendor changes to get your PR to work. The OpenStack plugin
