# Contributing to T-Cloud Plugin
...
runtime in order to build packer with the T-Cloud plugin.
...
## Setting up T-Cloud plugin for dev

With Go installed, you can already `go get` the T-Cloud plugin and `make dev` in
...
1. Download the T-Cloud plugin source (and its dependencies) by running
   `go get github.com/opentelekomcloud-community/packer-plugin-tcloud`. This will download the source to
   `$GOPATH/src/github.com/opentelekomcloud-community/packer-plugin-tcloud`.

2. When working on the T-Cloud plugin, first `cd $GOPATH/src/github.com/opentelekomcloud-community/packer-plugin-tcloud`
   so you can run `make dev` and easily access other files. `make dev` will build the packer-plugin-tcloud binary and install it under Packer's plugin directory.

3. Make your changes to the T-Cloud plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
   Any compilation errors will be shown when the binaries are rebuilding. If you don't have `make` you can run `go build -o packer-plugin-tcloud` from the project root, then install the plugin with `packer plugins install --path ./packer-plugin-tcloud github.com/opentelekomcloud-community/tcloud`.

4. After building the T-Cloud plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/opentelekomcloud-community/packer-plugin-tcloud/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the T-Cloud plugin](https://github.com/opentelekomcloud-community/packer-plugin-tcloud#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-tcloud f-my-feature
```
...
3. One of T-Cloud plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the T-Cloud plugin
project. After following the steps in "Setting up Go to work on the T-Cloud plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/opentelekomcloud-community/packer-plugin-tcloud`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-tcloud.git`
...
Please only apply the minimal vendor changes to get your PR to work. The T-Cloud plugin
...
If the OpenStack Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
