...
func (s *stepCreateImage) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	switch config.ImageCreationMethod {
	case imageCreationMethodOTC, imageCreationMethodOpenStack:
		// We need the v2 image client to wait for the created image. Validate it
		// before creating the image so endpoint failures do not leave an orphan.
	...
	default:
		err := fmt.Errorf("Unknown image_creation_method value %s", config.ImageCreationMethod)
		state.Put("error", err)
		ui.Error(err.Error())
		return multistep.ActionHalt
	}
	...
	switch config.ImageCreationMethod {
	case imageCreationMethodOTC:
		imageID, err = createOTCIMSImage(ctx, config.otcIMSClient(ctx, config.OTCIMSEndpoint), otcIMSCreateImageRequest{
			Name:                config.ImageName,
			InstanceID:          server.ID,
			Type:                "ECS",
			Tags:                config.ImageTags,
			EnterpriseProjectID: config.OTCEnterpriseProjectID,
		}, config.TenantID)
		if err != nil {
			err := fmt.Errorf("Error creating OTC IMS image: %s", err)
			state.Put("error", err)
			ui.Error(err.Error())
			return multistep.ActionHalt
		}
	case imageCreationMethodOpenStack:
